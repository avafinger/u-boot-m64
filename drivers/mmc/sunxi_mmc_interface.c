...
{
	MMCDBG("@lex - %s: dev %d, start %ld, blkcnt %ld src 0x%x\n", __FUNCTION__, dev_num, start, blkcnt, src);
...
static ulong mmc_bread_mass_pro(int dev_num, ulong start, lbaint_t blkcnt, void *dst)
{
    ...
    MMCINFO("%s: input parameter start: %lu\nCONFIG_MMC_LOGICAL_OFFSET: %d", __FUNCTION__, start,CONFIG_MMC_LOGICAL_OFFSET);

	if ((start == BOOT0_SDMMC_START_ADDR)
		&& ((start+blkcnt) < CONFIG_MMC_LOGICAL_OFFSET))
	{
		MMCINFO("%s: start read boot data...\n", __FUNCTION__);
		return mmc_get_boot(dev_num, start, blkcnt, dst);
	}
	else /*if (start > CONFIG_MMC_LOGICAL_OFFSET)*/
	{
		MMCINFO("%s: start read user data...\n", __FUNCTION__);
...
int mmc_init_blk_ops(struct mmc *mmc)
{
	...
	mmc->block_dev.block_read_secure_backup 	= sdmmc_secure_storage_read_backup; /* @lex */
