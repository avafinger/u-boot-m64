...
static int do_sunxi_bmp_info(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	else if(argc == 3)
	{
		char  load_addr[8];
		char  filename[32];
		char *const bmp_argv[6] = { "fatload", "mcc", getenv("boot_part") /* "0:1" */, load_addr, filename, NULL };
...
static int do_sunxi_bmp_display(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	else if(argc == 4)
	{
		char  load_addr[8];
		char  filename[32];
		char *const bmp_argv[6] = { "fatload", "mmc", getenv("boot_part") /* "0:1" */, load_addr, filename, NULL };
...
int sunxi_bmp_display(char *name)
{
	...
	//size_t file_size = 0;
	char * bmp_argv[6] = { "fatload", "mmc", getenv("boot_part") /* "0:1" */, "00000000", bmp_name, NULL };
